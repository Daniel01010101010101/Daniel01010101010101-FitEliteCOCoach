...
export default function LoginScreen() {
  ...
  const submit = async () => {
    setMessage("");
    if (!email || !password) return setMessage("Ingresa correo y contraseña.");
    const { error } = await authService.signIn(email, password);
    if (error) return setMessage(error.message || "No fue posible iniciar sesión.");
    router.replace("/dashboard");
  };

  return (
    <AuthLayout title="FitEliteCO Coach" subtitle="Inicia sesión o crea una cuenta para generar tu plan personalizado.">
