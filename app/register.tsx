...
export default function RegisterScreen() {
  ...
  const submit = async () => {
    setMessage("");
    if (!email.includes("@") || password.length < 6) return setMessage("Usa un correo válido y una contraseña de mínimo 6 caracteres.");
    const { error } = await authService.signUp(email, password);
    if (error) return setMessage(error.message || "No fue posible crear la cuenta.");
